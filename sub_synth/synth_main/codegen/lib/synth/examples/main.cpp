//
// Academic License - for use in teaching, academic research, and meeting
// course requirements at degree granting institutions only.  Not for
// government, commercial, or other organizational use.
// File: main.cpp
//
// MATLAB Coder version            : 4.0
// C/C++ source code generated on  : 17-Nov-2020 17:30:25
//

//***********************************************************************
// This automatically generated example C main file shows how to call
// entry-point functions that MATLAB Coder generated. You must customize
// this file for your application. Do not modify this file directly.
// Instead, make a copy of this file, modify it, and integrate it into
// your development environment.
//
// This file initializes entry-point function arguments to a default
// size and value before calling the entry-point functions. It does
// not store or use any values returned from the entry-point functions.
// If necessary, it does pre-allocate memory for returned values.
// You can use this file as a starting point for a main function that
// you can deploy in your application.
//
// After you copy the file, and before you deploy it, you must make the
// following changes:
// * For variable-size function arguments, change the example sizes to
// the sizes that your application requires.
// * Change the example values of function arguments to the values that
// your application requires.
// * If the entry-point functions return values, store these values or
// otherwise use them as required by your application.
//
//***********************************************************************
// Include Files
#include "rt_nonfinite.h"
#include "synth.h"
#include "main.h"
#include "synth_terminate.h"
#include "synth_emxAPI.h"
#include "synth_initialize.h"

// Function Declarations
static emxArray_boolean_T *argInit_1xUnbounded_boolean_T();
static void argInit_6x2_real_T(double result[12]);
static boolean_T argInit_boolean_T();
static double argInit_real_T();
static void main_synth();

// Function Definitions

//
// Arguments    : void
// Return Type  : emxArray_boolean_T *
//
static emxArray_boolean_T *argInit_1xUnbounded_boolean_T()
{
  emxArray_boolean_T *result;
  static int iv0[2] = { 1, 2 };

  int idx1;

  // Set the size of the array.
  // Change this size to the value that the application requires.
  result = emxCreateND_boolean_T(2, iv0);

  // Loop over the array to initialize each element.
  for (idx1 = 0; idx1 < result->size[1U]; idx1++) {
    // Set the value of the array element.
    // Change this value to the value that the application requires.
    result->data[result->size[0] * idx1] = argInit_boolean_T();
  }

  return result;
}

//
// Arguments    : double result[12]
// Return Type  : void
//
static void argInit_6x2_real_T(double result[12])
{
  int idx0;
  int idx1;

  // Loop over the array to initialize each element.
  for (idx0 = 0; idx0 < 6; idx0++) {
    for (idx1 = 0; idx1 < 2; idx1++) {
      // Set the value of the array element.
      // Change this value to the value that the application requires.
      result[idx0 + 6 * idx1] = argInit_real_T();
    }
  }
}

//
// Arguments    : void
// Return Type  : boolean_T
//
static boolean_T argInit_boolean_T()
{
  return false;
}

//
// Arguments    : void
// Return Type  : double
//
static double argInit_real_T()
{
  return 0.0;
}

//
// Arguments    : void
// Return Type  : void
//
static void main_synth()
{
  emxArray_real_T *y;
  emxArray_real_T *y2;
  emxArray_boolean_T *note;
  double fs;
  double f0;
  double Q;
  double wf;
  double N;
  double zi[12];
  double amp1;
  double amp2;
  emxInitArray_real_T(&y, 1);
  emxInitArray_real_T(&y2, 1);

  // Initialize function 'synth' input arguments.
  // Initialize function input argument 'note'.
  note = argInit_1xUnbounded_boolean_T();
  fs = argInit_real_T();
  f0 = argInit_real_T();
  Q = argInit_real_T();
  wf = argInit_real_T();
  N = argInit_real_T();

  // Initialize function input argument 'zi'.
  argInit_6x2_real_T(zi);

  // Call the entry-point 'synth'.
  synth(note, fs, f0, Q, wf, N, zi, argInit_real_T(), argInit_real_T(),
        argInit_real_T(), argInit_real_T(), argInit_real_T(), argInit_real_T(),
        argInit_real_T(), argInit_real_T(), argInit_real_T(), y, y2, &amp1,
        &amp2);
  emxDestroyArray_real_T(y2);
  emxDestroyArray_real_T(y);
  emxDestroyArray_boolean_T(note);
}

//
// Arguments    : int argc
//                const char * const argv[]
// Return Type  : int
//
int main(int, const char * const [])
{
  // Initialize the application.
  // You do not need to do this more than one time.
  synth_initialize();

  // Invoke the entry-point functions.
  // You can call entry-point functions multiple times.
  main_synth();

  // Terminate the application.
  // You do not need to do this more than one time.
  synth_terminate();
  return 0;
}

//
// File trailer for main.cpp
//
// [EOF]
//
